...
def func(i,j):
    ...
    sigma=theta+((height/2)-j)*(46/height)
    sigma_rad=((math.pi/180)*sigma)
    phie=(i-width/2)*(88/width)
    phie_rad=((math.pi/180)*phie)
    h=1.5
    Y=1.5*math.tan(sigma_rad)
    X=Y*math.tan(phie_rad)
    Z=math.sqrt(h*h+Y*Y+X*X)
    ...
    def draw_humans(npimg, humans, imgcopy=False):
        ...
        for human in humans:
            ...
            for i in range(common.CocoPart.Background.value):
                ...
                Value=[]
                Value.append(body_part.part_idx)
                
                
                center = (int(body_part.x * image_w + 0.5), int(body_part.y * image_h + 0.5))
#                print("3D coordinates of the point"+str(body_part.part_idx)+"are",func(center[0],center[1]))
                centers[i] = center
                ...
                cv2.putText(npimg,str(body_part.part_idx),(x,y), cv2.FONT_HERSHEY_TRIPLEX, 1.0, (255, 255, 255))
            ...
            for val in Values:
                if val[0]==2:
                    ...
                    Rshoulder=(val[1],val[2])
                    
                if val[0]==5:
                    Lshoulder=(val[1],val[2])
                if val[0]==1:
                    Neck.append((val[1],val[2]))                
                if val[0]==8:
                    Rhip=(val[1],val[2])
                if val[0]== 11:
                    Lhip=(val[1],val[2])
                ...
                if val[0]==0:
                    Nose=(val[1],val[2])
#            if Rshoulder or Lshoulder is not None:
#                Shoulder_distance=math.sqrt(sum([(a-b)**2 for a,b in zip(Rshoulder,Lshoulder)]))
#                print("Shoulder distance=",Shoulder_distance)
#            if Rshoulder or Rhip is not None:
#                shirt_height=math.sqrt(sum([(a-b)**2 for a,b in zip(Rshoulder,Rhip)]))
#                print("Shirt size=",shirt_height)
#            if Rhip or Lhip is not None:
#                waist_size=math.sqrt(sum([(a-b)**2 for a,b in zip(Rhip,Lhip)]))
#                print("Waist size=",waist_size)
#            if Rshoulder or Relbow is not None:
#                elbow_length=math.sqrt(sum([(a-b)**2 for a,b in zip(Rshoulder,Relbow)]))
#                print("elbow_length=",elbow_length)
#            if Relbow or Rwrist is not None:
#                elbow_wrist=math.sqrt(sum([(a-b)**2 for a,b in zip(Rwrist,Relbow)]))
#                print("elbow_wrist=",elbow_wrist)
#            if Rhip or Rankle is not None:
#                leg_distance=math.sqrt(sum([(a-b)**2 for a,b in zip(Rhip,Rankle)]))
#                print("leg_distance=",leg_distance)
#                
